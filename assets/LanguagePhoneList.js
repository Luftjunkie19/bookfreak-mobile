...
import countriesTranslations from './translations/CountriesTranslations.json';

const LanguagePhoneList = () => {
    const selectedLanguage=useSelector((state)=>state.languageSelection.selectedLangugage);
    // length 252
const worldCountriesPhoneList= [
...
const countriesArray=worldCountriesPhoneList.map((item)=>{
   let languages=[];
    if(countriesTranslations[`${item.name}`]){
     languages.push({...item, name: countriesTranslations[`${item.name}`][selectedLanguage]});
    }

    return languages;
})

return {countries: countriesArray, worldCountries: worldCountriesPhoneList}
}

export default LanguagePhoneList